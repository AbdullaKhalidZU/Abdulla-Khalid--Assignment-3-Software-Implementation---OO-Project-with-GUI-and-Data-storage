# -*- coding: utf-8 -*-
"""Full code with Supplier GUI window test cases
...
class ManagementApp:
    def __init__(self, master):
        ...
        self.label = tk.Label(master, text="Supplier ID:")
        ...
        self.add_supplier_button.grid(row=1, column=0)

        self.delete_supplier_button = tk.Button(master, text="Delete Supplier", command=self.delete_supplier)
        self.delete_supplier_button.grid(row=1, column=1)

        self.modify_supplier_button = tk.Button(master, text="Modify Supplier", command=self.modify_supplier)
        self.modify_supplier_button.grid(row=1, column=2)

        self.display_supplier_button = tk.Button(master, text="Display Supplier", command=self.display_supplier)
        self.display_supplier_button.grid(row=1, column=3)

        # List to store supplier instances
        self.suppliers = []

        # Initialize suppliers from provided data
        self.initialize_suppliers()

    def initialize_suppliers(self):
        # Initialize suppliers from provided data
        # Supplier data provided: supplier_id, name, address, contact_details
        supplier_data = [
            (1, "Supplier 1", "Address 1", "123-456-7890"),
            (2, "Supplier 2", "Address 2", "456-789-0123"),
            (3, "Supplier 3", "Address 3", "789-012-3456"),
        ]
    ...
    def add_supplier(self):
        ...
        contact_details = input("Enter supplier contact details: ")
    ...
    def modify_supplier(self):
        supplier_id = int(input("Enter supplier ID to modify: "))
        for supplier in self.suppliers:
            if supplier.get_supplier_id() == supplier_id:
                print("Modify Supplier:")
                name = input(f"Enter new name (current: {supplier.get_name()}): ")
                address = input(f"Enter new address (current: {supplier.get_address()}): ")
                contact_details = input(f"Enter new contact details (current: {supplier.get_contact_details()}): ")

                supplier.set_name(name)
                supplier.set_address(address)
                supplier.set_contact_details(contact_details)

                print("Supplier modified successfully!")
                return
        print("Supplier not found.")

    def display_supplier(self):
        supplier_id = int(input("Enter supplier ID to display: "))
        for supplier in self.suppliers:
            if supplier.get_supplier_id() == supplier_id:
                print("Supplier Details:")
                print(f"Supplier ID: {supplier.get_supplier_id()}")
                print(f"Name: {supplier.get_name()}")
                print(f"Address: {supplier.get_address()}")
                print(f"Contact Details: {supplier.get_contact_details()}")
                return
        print("Supplier not found.")
